    ...
    return {
        ...
        resolveId(source, importer) {
            ...
            if (extname(source) === '.vue') {
                ...
                if (!isPage) {
                    // 将绝对路径转换为相对路径（相对于项目根目录），避免 Windows 上 ESM loader 的路径问题
                    // 使用相对路径而不是绝对路径，因为 Vite 的 ESM loader 在 Windows 上需要 file:// URL 或相对路径
                    const relativePath = relative(root, fullPath)
                    const normalizedRelativePath = normalize(relativePath)
                    const virtualId = VIRTUAL_PREFIX + changeExt(normalizedRelativePath, '.vue', VIRTUAL_EXT)
                    if (debug) {
        ...
        async load(id) {
            ...
            if (id.startsWith(VIRTUAL_PREFIX) && id.endsWith(VIRTUAL_EXT)) {
                ...
                // tempPath 是相对路径（相对于项目根目录），需要转换为绝对路径
                const relativePath = changeExt(tempPath, VIRTUAL_EXT, '.vue')
                const originalPath = resolve(root, relativePath)
                ...
                // 添加 CSS 导入（使用相对路径避免 Windows 路径问题）
                let finalTsCode = tsCode
                if (styles.trim()) {
                    const relativeOriginalPath = relative(root, originalPath)
                    const normalizedRelativePath = normalize(relativeOriginalPath)
                    finalTsCode = `import '${CSS_VIRTUAL_IMPORT_PREFIX}${normalizedRelativePath}.css'\n${finalTsCode}`
                }
        ...
        transform(code, id) {
            ...
            // 使用相对路径（相对于项目根目录）避免 Windows 上 ESM loader 的路径问题
            const relativeId = relative(root, id)
            const normalizedRelativeId = normalize(relativeId)
            let finalCode = result.code
            if (result.styles.trim()) {
                // 在 <script> 开始标签后插入 CSS 导入
                finalCode = finalCode.replace(
                    /(<script[^>]*>)/,
                    `$1\nimport '${CSS_VIRTUAL_IMPORT_PREFIX}${normalizedRelativeId}.css'`
                )
            }
        ...
        handleHotUpdate({ file, server }) {
            ...
            if (!isPage) {
                ...
                // CSS 虚拟模块也需要失效（使用相对路径）
                const cssVirtualId = CSS_VIRTUAL_ID_PREFIX + normalizedRelativePath + '.css'
